...
class _Plot3ScreenState extends State<Plot3Screen> {
  ...
  String? _normalizeOwnerId(dynamic rawOwner) {
    ...
    if (rawOwner is Map) {
      final dynamic possibleUserId = rawOwner['userId'] ?? rawOwner['_id'] ?? rawOwner['id'];
  ...
  Future<void> _getOwnerIdFromWorker() async {
    ...
    print('🔍 DEBUG: URL ที่เรียก: http://10.0.2.2:3000/api/profile/worker-info/${widget.userId}');
  ...
  Future<void> _loadPlotData() async {
    ...
    print('🔍 DEBUG: URL ที่เรียก: http://10.0.2.2:3000/api/plots/owner/$ownerId');

    try {
      // ดึงแปลงปลูกของเจ้าของด้วย endpoint ใหม่
      final response = await http.get(
        Uri.parse('http://10.0.2.2:3000/api/plots/owner/$ownerId'),
        headers: {"Content-Type": "application/json"},
      );
      ...
      if (response.statusCode == 200) {
        ...
        print('❌ โหลดแปลงปลูกไม่สำเร็จ: ${response.statusCode} - ${response.body}');
      }
