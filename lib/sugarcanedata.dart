  ...
  Future<Map<String, dynamic>> _fetchRecommendations() async {
    try {
      final response = await http
          .get(
            Uri.parse(
                'https://sugarcane-9bacy8d0d-suphachais-projects-d3438f04.vercel.app/api/plots/${widget.plotId}/recommendations'),
          )
                          ...
                          if (isSaved) {
                            ...
                            try {
                              final response = await http.get(
                                Uri.parse(
                                    'https://sugarcane-9bacy8d0d-suphachais-projects-d3438f04.vercel.app/api/plots/${widget.plotId}/recommendations/${topic.title}'),
                              );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final tasksResponse = await http.get(
          Uri.parse('https://sugarcane-9bacy8d0d-suphachais-projects-d3438f04.vercel.app/api/plots/${widget.plotId}/tasks'),
        );
...
// ดึงชื่อคนงานจาก workerId
  Future<void> _fetchWorkerNames(List<dynamic> tasks) async {
    ...
    for (final workerId in workerIds) {
      try {
        final response = await http.get(
          Uri.parse('https://sugarcane-9bacy8d0d-suphachais-projects-d3438f04.vercel.app/api/workers/$workerId'),
        );
  ...
  Future<void> _saveData() async {
    ...
    try {
      ...
      final response = widget.isEditing
          ? await http.put(
              Uri.parse(
                  'https://sugarcane-9bacy8d0d-suphachais-projects-d3438f04.vercel.app/api/plots/${widget.plotId}/recommendations/${widget.topic}'),
              headers: {'Content-Type': 'application/json'},
              body: jsonEncode(requestData),
            )
          : await http.post(
              Uri.parse(
                  'https://sugarcane-9bacy8d0d-suphachais-projects-d3438f04.vercel.app/api/plots/${widget.plotId}/recommendations'), // ✅ ต้องมี s
              headers: {'Content-Type': 'application/json'},
              body: jsonEncode(requestData),
            );
      ...
      if (widget.isWorker && widget.taskId != null) {
        try {
          final taskResponse = await http.put(
            Uri.parse(
                'https://sugarcane-9bacy8d0d-suphachais-projects-d3438f04.vercel.app/api/plots/${widget.plotId}/tasks/${widget.taskId}/status'),
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode({
              'status': 'completed',
